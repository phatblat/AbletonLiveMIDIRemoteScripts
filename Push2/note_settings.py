...
# File: note_settings.pyc (Python 2.5)
...
class NoteSetting(NoteSettingBase):
    ...
    min = listenable_property(min)
    
    def max(self):
        return self._get_min_max_value(1)

    max = listenable_property(max)
...
class NoteSettingsComponent(NoteSettingsComponentBase):
    ...
    def __init__(self, *a, **k):
        super(NoteSettingsComponent, self).__init__(*a, **a)
        self._NoteSettingsComponent__on_color_index_changed.subject = self.song.view

    
    def _create_settings(self, grid_resolution):
        args = dict(grid_resolution = grid_resolution)
        self._nudge = NoteNudgeSetting(**None)
        self._coarse = NoteLengthCoarseSetting(**None)
        self._fine = NoteLengthFineSetting(**None)
        self._velocity = NoteVelocitySetting(**None)
    ...
    nudge = property(nudge)
    
    def coarse(self):
        return self._coarse

    coarse = property(coarse)
    ...
    color_index = listenable_property(color_index)
    
    def __on_color_index_changed(self, *a):
        self.notify_color_index()

    _NoteSettingsComponent__on_color_index_changed = listens('detail_clip.color_index')(__on_color_index_changed)
