...
class _SimplerDeviceDecorator(Subject, SlotManager, LiveObjectDecorator):
    ...
    def setup_parameters(self):
        ...
        self.voices_param = EnumWrappingParameter(name = 'Voices', parent = self, values_property = 'available_voice_numbers', index_property = 'voices', €Fz = lambda i: self.available_voice_numbers[i], -ñ© = lambda i: self.available_voice_numbers.index(i), value_type = int)
        ...
        self.gain_param = WrappingParameter(name = 'Gain', parent = self._live_object, source_property = 'gain', PVñ© = lambda _: if liveobj_valid(self._live_object):
pass1'')
    ...
    def setup_options(self):
        ...
        self.warp_as_x_bars_option = DeviceTriggerOption(name = 'Warp as X Bars', default_label = self.get_warp_as_option_label(), €Fz = lambda : call_simpler_function('warp_as', call_simpler_function('guess_playback_length')),  Nz = lambda : get_simpler_flag('can_warp_as'))
        self.warp_half_option = DeviceTriggerOption(name = ':2', callback = partial(call_simpler_function, 'warp_half'), €Fz = lambda : get_simpler_flag('can_warp_half'))
        self.warp_double_option = DeviceTriggerOption(name = 'x2', callback = partial(call_simpler_function, 'warp_double'), @‚z = lambda : get_simpler_flag('can_warp_double'))
