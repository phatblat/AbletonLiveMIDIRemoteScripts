...
class _SimplerDeviceDecorator(Subject, SlotManager, LiveObjectDecorator):
    ...
    def setup_parameters(self):
        ...
        self.voices_param = EnumWrappingParameter(name = 'Voices', parent = self, values_property = 'available_voice_numbers', index_property = 'voices', €6$ = lambda i: self.available_voice_numbers[i], à Bñ” = lambda i: self.available_voice_numbers.index(i), value_type = int)
        ...
        self.gain_param = WrappingParameter(name = 'Gain', parent = self._live_object, source_property = 'gain',  ÉBñ” = lambda _: if liveobj_valid(self._live_object):
pass1'')
    ...
    def setup_options(self):
        ...
        self.warp_as_x_bars_option = DeviceTriggerOption(name = 'Warp as X Bars', default_label = self.get_warp_as_option_label(), €6$ = lambda : call_simpler_function('warp_as', call_simpler_function('guess_playback_length')),  >$ = lambda : get_simpler_flag('can_warp_as'))
        self.warp_half_option = DeviceTriggerOption(name = ':2', callback = partial(call_simpler_function, 'warp_half'), €6$ = lambda : get_simpler_flag('can_warp_half'))
        self.warp_double_option = DeviceTriggerOption(name = 'x2', callback = partial(call_simpler_function, 'warp_double'), @r$ = lambda : get_simpler_flag('can_warp_double'))
