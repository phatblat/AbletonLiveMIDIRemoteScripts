...
# File: device_parameter_bank.pyc (Python 2.5)
...
class DeviceParameterBank(SlotManager, Subject):
    ...
    def __init__(self, size = None, device = None, banking_info = None, *a, **k):
        ...
        super(DeviceParameterBank, self).__init__(*a, **a)
        self._size = size
        self._device = device
    ...
    parameters = listenable_property(parameters)
...
class DescribedDeviceParameterBank(DeviceParameterBank):
    ...
    def __init__(self, device = None, banking_info = None, *a, **k):
        ...
        self._dynamic_slots = []
        super(DescribedDeviceParameterBank, self).__init__(device = device, banking_info = banking_info, *a, **a)
        self._update_parameters()
    ...
    _on_slot_content_changed = listens_group('content')(_on_slot_content_changed)
    
    def _current_parameter_slots(self):
        if not self._definition.value_by_index(self.index).get(PARAMETERS_KEY):
            pass
        return tuple()
    ...
    def _collect_parameters(self):
        ...
        continue
        return [ (find_if,)(lambda p: p.original_name == str(slot_definition), parameters) for None in bank_slots ]
...
class MaxDeviceParameterBank(DeviceParameterBank):
    ...
    def __init__(self, *a, **k):
        super(MaxDeviceParameterBank, self).__init__(*a, **a)
    ...
    def _collect_parameters(self):
        ...
        continue
        return [ None for index in indices ]
