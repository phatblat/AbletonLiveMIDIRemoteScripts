...
# File: observable_property_alias.pyc (Python 2.5)
...
class ObservablePropertyAlias(SlotManager):
    ...
    def __init__(self, alias_host, property_host = None, property_name = '', alias_name = None, getter = None, *a, **k):
        super(ObservablePropertyAlias, self).__init__(*a, **a)
        self._alias_host = alias_host
        if not alias_name:
            pass
        self._alias_name = property_name
    ...
    def _setup_alias(self, getter):
        if not getter:
            pass
        aliased_prop = property(self._get_property)
