...
# File: device_component.pyc (Python 2.5)
...
class DeviceComponentWithBank(ProviderDeviceComponent, CompoundComponent):
    ...
    def __init__(self, banking_info = None, *a, **k):
        ...
        super(DeviceComponentWithBank, self).__init__(*a, **a)
    ...
    _on_bank_parameters_changed = listens('parameters')(_on_bank_parameters_changed)
...
class DeviceComponent(DeviceComponentWithBank):
    ...
    def __init__(self, device_decorator_factory = None, *a, **k):
        if not device_decorator_factory is not None:
            raise AssertionError
        super(DeviceComponent, self).__init__(*a, **a)
        ...
        self._DeviceComponent__on_waveform_channel_changed.subject = self._waveform_real_time_data
        self._DeviceComponent__on_playhead_channel_changed.subject = self._playhead_real_time_data
    ...
    def set_device(self, device):
        if self._device_changed(device):
            ...
            if is_simpler(decorated_device):
                pass
            1
            self._DeviceComponent__on_file_path_changed.subject = None
    ...
    _DeviceComponent__on_waveform_channel_changed = listens('channel_id')(__on_waveform_channel_changed)
    
    def __on_playhead_channel_changed(self):
        self._update_real_time_channel('playhead')

    _DeviceComponent__on_playhead_channel_changed = listens('channel_id')(__on_playhead_channel_changed)
    ...
    _on_simpler_zoom_changed = listens('zoom')(_on_simpler_zoom_changed)
    
    def __on_file_path_changed(self):
        self._waveform_real_time_data.invalidate()

    _DeviceComponent__on_file_path_changed = listens('sample_file_path')(__on_file_path_changed)
    
    def _has_simpler_in_multi_sample_mode(self):
        device = self.device()
        if is_simpler(device):
            pass
        return device.multi_sample_mode
    ...
    def _create_parameter_info(self, parameter):
        ...
        def is_available(param):
            ...
            1
            name = ''
            if not not self._has_simpler_in_multi_sample_mode():
                pass
            return name not in self.ZOOMABLE_PARAMETERS + ('Zoom',)

        if is_available(parameter):
            pass
        1
        return parameter(None, default_sens_factory = self.default_sensitivity, fine_sens_factory = self.fine_sensitivity)
    ...
    def options(self):
        return getattr(self._bank, 'options', [
    ...
    _DeviceComponent__on_options_changed = listens('options')(__on_options_changed)
    
    def _setup_bank(self, device):
        super(DeviceComponent, self)._setup_bank(device, bank_factory = create_device_bank_with_options)
        
        try:
            self._DeviceComponent__on_options_changed.subject = self._bank
        except SlotError:
            pass
